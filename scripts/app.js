async function main() {
  ...
  async function updateMintSlider(userAddress) {
	const mintSlider = document.getElementById('mintSlider');
  
	// Hämta priset direkt i updateMintSlider
	const priceInWei = await contract.methods.publicPrice().call({ from: userAddress });
	const tokenPrice = web3.utils.fromWei(priceInWei, 'ether');
  ...
  async function _setupWeb3ConnectionMint(contractAddress, contractABI) {
	const web3 = new Web3(window.ethereum);
	const contract = createContractInstance(web3, contractAddress, contractABI);
  ...
  async function onConnectClick() {
	...
	await updateMintSlider(userAddress);
